...
#define minimum_delete_size 0x40000000  //1024M
...
static int change_file(stream_info_t *stream_info)
{
	...
	stream_info->max++;
#if NEW_FOLDER_FUNC
	stream_info->max = cam_get_new_file_name(stream_info->fh, filename, stream_info->ext, g_packer_info->name_format ? stream_info->file_start_time:(long)stream_info->max);
#else	
	...
	stream_info->packer->set(stream_info->handle_packer, MUX_PATH_TEMP_FILE, filename);
	...
	if (stream_info->reserve_size)
	{
		...
		if (0 > truncate(stream_info->filename, stream_info->reserve_size))
		{
			printf("truncate fail, try delete more file\n");
			del_file(g_packer_info);
			if (0 > truncate(stream_info->filename, stream_info->reserve_size))
					printf("truncate fail again\n");
				else
					printf("truncate OK\n");
		}
		g_packer_info->space_left -= stream_info->reserve_size;
	}

	free_space = g_packer_info->space_left + stream_info->reserve_size;
...
static int del_file(loop_packer_info_t *ptr_loop_info)
{
	...
	while(del_space < minimum_delete_size)
	{
		int delete_this_time = 0;
...
#if NEW_FOLDER_FUNC
				int ret;
				ret = cam_delete_file(stream->fh, 0, stream->filename);
				del_space += (ret < 0 ? 0:ret);
				delete_this_time += (ret < 0 ? 0:ret);
#else				
				do
				{
					struct stat stbuf;
	...
	while(1)
	{
		...
		switch(command->command)
		{
			...
			case SYNC_LOCK_FILE:
				...
				sleep_cnt=0;
				while(!g_packer_info->stop_sleep && sleep_cnt<10 && g_packer_info->stream_num>=2) {
					sleep_cnt++;
					sleep(1);
				}					
...
static SINT32 loop_packer_close(SINT32 hd)
{
	...
	if(stream->cur_file)
	{
		...
		if (stream->pkg_size == 0 || stream->frame_info.frame_time < 60)
		{
			unlink(stream->filename);
			printf("empty video, delete %s\n", stream->filename);
		}
...
static SINT32 loop_packer_open(HANDLE out)
{
	...
	stream->max = cam_get_new_file_name(stream->fh, filename, stream->ext, g_packer_info->name_format ? stream->file_start_time:(long)stream->max);
	if (stream->max < 0)
	{
	...
	if (stream->reserve_size)
		...
		truncate(filename, stream->reserve_size);
	...
	stream->packer->set(stream->handle_packer, MUX_PATH_TEMP_FILE, filename);
...
#else
			if (stream_info->frame_info.frame_time)
				change_file(stream_info);
#endif			
...
static SINT32 loop_packer_set(SINT32 hd, UINT32 param, UINT32 value)
{
	...
	else if (param == MUX_FILE_START_TIME)
	{
		if ((g_packer_info->file_start_time < value && (value - g_packer_info->file_start_time) < 3) ||
			(g_packer_info->file_start_time > value && (g_packer_info->file_start_time - value) < 3))
			value = g_packer_info->file_start_time;
		stream_info->file_start_time = value;
		g_packer_info->file_start_time = value;
		return 0;
	}
	else if (param == MUX_PATH_TEMP_FILE)
		stream_info->record_arg[param] = 0; // don't save

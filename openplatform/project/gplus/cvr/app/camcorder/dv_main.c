...
int plateNumber_hd = -1;
char plateNumber[11] = {'\0'};
int plateNumber_hd_ch = -1;
char plateNumberCh[4] = {'\0'};
...
int dv_parse_command(int argc, char *argv[])
{
	...
	for (; i < argc; i++)
	{
		...
		else if (strcmp(argv[i], "-loop") == 0)
		{
			...
			stream.attr.time_length = opt.loop_file_time*60;
		}
...
static void add_time_stamp(streamInfo_t* info)
{
	char string[32];
	int font_type;
	...
	gp_OSD_AddTimeStamp(info->handle, opt.time_format, x, y, font_type, 0);
	gp_OSD_Enable(info->handle, 1);
}
static int add_string(streamInfo_t* info, int x, int y, char *string, int font_type, int ch)
{
	//char string[32];
	int ret = -1;
	//int font_type;
	...
	if(ch) {
		gp_OSD_GetTextSizeCh(string, font_type, &w, &h, string[0]);
	}
	else {
		gp_OSD_GetTextSize(string, font_type, &w, &h);
	}
	...
	if(ch) {
		printf("add ch idx %d\n", string[1]);
		ret = gp_OSD_AddTextCh(info->handle, &string[1], x, y, string[0], font_type);
	}
	else {
		printf("add text %s\n", string);
		ret = gp_OSD_AddText(info->handle, string, x, y, font_type);
	}
	gp_OSD_Enable(info->handle, 1);
	return ret;
}

static int remove_string(streamInfo_t *info, int handle)
{
	return gp_OSD_RemoveText(info->handle, handle);
}


static int add_plateNumber(streamInfo_t *info)
{
	...
	if(plateNumber[0] == '0' || plateNumber[0] == '\0') {
		if(plateNumber_hd != -1) {
			remove_string(&stream, plateNumber_hd);
			plateNumber_hd = -1;
		}
		if(plateNumber_hd_ch != -1) {
			remove_string(&stream, plateNumber_hd_ch);
			plateNumber_hd_ch = -1;
		}
	}
	else {
		...
		if(plateNumber[0] == '1') {
			plateNumber_hd = add_string(&stream, x, y, &plateNumber[1], font_type, 0);
		}
		else if(plateNumber[0] == '2') {
			plateNumberCh[0] = plateNumber[1];
			plateNumberCh[1] = plateNumber[2]-1;
			printf("%d %d\n", plateNumberCh[0], plateNumberCh[1]);
			plateNumber_hd_ch = add_string(&stream, x, y, plateNumberCh, font_type, 1);
			plateNumber_hd = add_string(&stream, x+20*(font_type+1), y, &plateNumber[3], font_type, 0);
		}
	}
}
...
static int enable_motion_detect()
{
	...
	gd_md_open(MD_SENSITIVITY_HIGH);
...
======================================*/			
	while(1)
	{
		...
		/* receive UI command */
		if(continue_recording || g_bSaveFail || (opt.fifo && dvr_pipemsg_receive(&msgId, &msgPara) > 0))
		{
			if (continue_recording)
			{
				continue_recording = 0;
				msgId = CMD_START_DV;
				dvr_pipemsg_send(CMD_SET_LOOPRECORDING_INT, 1, &continue_recording);
			}
			...
			case CMD_SET_DV_RESOLUTION:
				...
				if (stream.attr.rsl != rsl || stream.attr.framerate != fps)
				{
					...
					printf("[dvr] change mode to %d x %d %dfps\n", stream.width, stream.height, stream.attr.framerate);
			...
			case CMD_SET_LOOPRECORDING:
				...
				stream.attr.time_length = mode ? opt.loop_file_time*60 : 0;
				break;
			case CMD_SET_MD:
			...
			case CMD_SET_LDW_EN:
				{
						...
						(opt.ldw == 0 && ldw.LDW_Enable > 0))
					{
						...
						if (stream.attr.rsl == IPC_RESOLUTION_1080P)
						{
							g_bRestart = 1;
							g_bQuit = 1;
						}
			...
			case CMD_SET_DISPLAY_MODE:
				...
				break;
			case CMD_SET_PLATE_NUMBER:
				printf("set plate enable:%d\n", ((char*)msgPara)[0]);
				memcpy(plateNumber, (char *)msgPara, 10);
				if(plateNumber[0] == 2) {
					//plateNumber[0] = 1;
					//plateNumberCh[1] -= 1;
				}

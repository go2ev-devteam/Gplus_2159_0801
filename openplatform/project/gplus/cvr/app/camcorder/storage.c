		...
		if(msg->cmd == STORAGE_CMD_WRITE || msg->cmd == STORAGE_CMD_WRITE_AUDIO) {
			...
			if (storageHandle && storageHandle->hdMux) {
				...
				else
				{
					...
					if (msg->frameType == vid_FRAME_START_I && storageHandle->hdMux)
						g_save_Mux->set(storageHandle->hdMux, MUX_FILE_START_TIME, (unsigned int)(msg->pts/1000));

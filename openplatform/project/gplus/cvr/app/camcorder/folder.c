...
static int get_date_folder_name(char* foldername, char* path, time_t t)
{
	struct tm tm;
	int year,month,day;
...
static int get_index_file_name(char* filename, long index, char* path, char* ext)
{
	struct tm tm;
	int hour, minute;
	...
	sprintf(filename, "%s/%02d%02d%04ld.%s", path, hour, minute, index, ext);
	
	return 0;
}

static int get_time_file_name(char* filename, time_t t, char* path, char* postfix, char* ext)
{
	struct tm tm;
...
static int get_current_folder2(folderInfo_t* fi, char* folderpath, time_t t)
{
	char path[256];
	int fnum = 0;
	DIR *dir;
	
	get_date_folder_name(path, fi->working_path, t);
...
int cam_get_new_file_name(void* handle, char* filename, char* ext, long idx)
{
	...
	if (fi->name_format == 1)
	{
		ret = get_current_folder2(fi, folder, (time_t)idx);

		if (0 > ret)
			return -1;
			
		get_time_file_name(filename, (time_t)idx, folder, fi->postfix, ext);
...
int cam_delete_file(void* handle, int size, char* except_file_name)
{
	folderInfo_t* fi = (folderInfo_t*)handle;
	char folder[256];
	char file[256];
	...
	while(1)
	{
		...
		for (i = fi->delete_idx; i < fi->old_file_num; i++)
		{
			...
			if (except_file_name && strcmp(file, except_file_name) == 0)
				continue;
		...
		if (del_space >= size || fi->delete_folder_idx == fi->folder_num)
			break;
	}

...
//	font_data = 0;
	...
	if(str_info->pos_x +font.font_width*HDMIxNum > (320-font.font_width*HDMIxNum) && flag_AutoNextLine)
	{
		str_info->pos_x=font.font_width*HDMIxNum;
		str_info->pos_y+=font.font_height;
	}
	if (len) {
...
INT32S ap_state_resource_string_draw_one(INT16U *frame_buff, STRING_INFO *str_info,int index)
{
	INT32U str_addr, iStrLoc;
	INT16U character = 0;
	...
	if (!str_addr) {
   	...
   	character = ((INT16U *) str_addr)[index];
	if (ap_state_resource_char_draw(character, frame_buff, str_info, RGB565_DRAW, 0)) {
		gp_free((void *) str_addr);
		DBG_PRINT("Failed to draw char font\r\n");
		return STATUS_FAIL;

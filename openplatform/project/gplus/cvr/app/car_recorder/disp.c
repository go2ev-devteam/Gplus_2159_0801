...
iconManager_t Icon_Manager[ICON_MAX]={
	...
	{"SFCW.bin",	   ICON_FCW			 ,	ICON_WIDTH,ICON_HEIGHT, ICON_LEFT_X_POS,ICON_RIGHT_Y_POS,				   TRANSPARENT_COLOR,NULL,0},	  
	{"StopAndGo.bin",	   ICON_STOPandGO		,	ICON_WIDTH,ICON_HEIGHT, ICON_LEFT_X_POS,ICON_RIGHT_Y_POS+ICON_HEIGHT,			   TRANSPARENT_COLOR,NULL,0},	  
	{"ADAS.bin",	   ICON_ADAS		  , ICON_WIDTH,ICON_HEIGHT, UI_MENU_X_POS+X_STEP,UI_MENU_Y_POS+Y_STEP+8,					TRANSPARENT_COLOR,NULL,0},	   
...
#if CVR_PLATE_NUMBER
	{"DateTime.bin",		ICON_DATE_TIME_SET1    ,	ICON_WIDTH,ICON_HEIGHT,	UI_MENU_X_POS+X_STEP,UI_MENU_Y_POS+4*Y_STEP+8,						TRANSPARENT_COLOR,NULL,0},            
#endif

#if ADAS_CALIBRATE
	{"calibration.bin", 	ICON_GSENSOR_CALIBRATE		   ,	ICON_WIDTH,ICON_HEIGHT, UI_MENU_X_POS+X_STEP,UI_MENU_Y_POS+Y_STEP+8,					TRANSPARENT_COLOR,NULL,0},	   
	{"Arrowup.bin", 	ICON_GSENSOR_CALIBRATE		   ,	ICON_WIDTH,ICON_HEIGHT, ICON_LEFT_X_POS,UI_MENU_Y_POS,					TRANSPARENT_COLOR,NULL,0},	   
	{"Arrowdown.bin", 	ICON_GSENSOR_CALIBRATE		   ,	ICON_WIDTH,ICON_HEIGHT, ICON_LEFT_X_POS,UI_MENU_Y_POS,					TRANSPARENT_COLOR,NULL,0},	   
#endif	
	
	{"Beep_on.bin",		ICON_BEEP_ON 			 ,	ICON_WIDTH,ICON_HEIGHT, ICON_RIGHT_X_POS,ICON_RIGHT_Y_POS+4*ICON_HEIGHT,				TRANSPARENT_COLOR,NULL,0},			 
	{"Beep_off.bin", 	ICON_BEEP_OFF			  , ICON_WIDTH,ICON_HEIGHT, ICON_RIGHT_X_POS,ICON_RIGHT_Y_POS+4*ICON_HEIGHT,				TRANSPARENT_COLOR,NULL,0},			 
};
...
menuManager_t DV_SETTING[DV_SETTING_NUM] = {
		...
		//car num
		{
			.icon_Index = ICON_DATE_TIME_SET1,
			.str_index =	STR_CAR_NUM,
			.color = FONT_COLOR,
			.item_num = 2,	
			.item = 
			{
...
///////////////no adas menu////////////
	...
	#if (LDW_FCW && LDW_STOPANDGO)
				{
					.str_index = STR_LDW_FCW,
					.xPos = 220,
					.yPos = UI_MENU_Y_POS+2*Y_STEP,
				},
				{
					.str_index = STR_LDW_STOPAndGO,
					.xPos = 220,
					.yPos = UI_MENU_Y_POS+3*Y_STEP,
				},	
				#if ADAS_CALIBRATE
				{
					.str_index = STR_ADAS_CALIBRATE,
					.xPos = 220,
					.yPos = UI_MENU_Y_POS+4*Y_STEP,
				},	
				{
					.str_index = STR_ADAS_SOUND,
					.xPos = 220,
					.yPos = UI_MENU_Y_POS+1*Y_STEP,
				},	
				#else
...
///////////////////////////////////////////////////
				...
				{
					.str_index = STR_ADAS_GSENSOR_PARA,
					.xPos = 220,
					.yPos = UI_MENU_Y_POS+2*Y_STEP,
				},
				{
					.str_index = STR_ADAS_INSTALL_GUIDE,
					.xPos = 220,
					.yPos = UI_MENU_Y_POS+3*Y_STEP,
				},
				#endif
...
char dt_str1[] = 	"00/00/2000   00:00:00";
...
)
{
	...
	{	
		...
		dispOutput.mode = SP_DISP0_TV_MODE_NTSCJ;
		else
		dispOutput.mode = SP_DISP0_TV_MODE_PAL;	
	}
	#endif
...
ioctl(pDisp->fdDisp, DISPIO_SET_GAMMA_ENABLE, 0 );
	...
	if (pDisp->layer == DISP_LAYER_PRIMARY) {
		...
		else if(dis_type == SP_DISP_OUTPUT_LCD)
		{
			...
			osdScale.width = RESOLUTION_WIDTH;
			osdScale.height = RESOLUTION_HEIGHT;					
		}
		#ifdef SYSCONFIG_DISP0_TVOUT
		else if(dis_type == SP_DISP_OUTPUT_TV)
		{
...
UINT32 dispSetTVMode()
{
	dispDestroy(hDisp);
	dispCreate(&hDisp, DISP_LAYER_OSD,dv_set.display_mode);
}
UINT32
dispDestroy(
	HANDLE handle
...
void dispEnablePrimary(HANDLE handle)
{
	dispManager_t *pDisp = (dispManager_t *)handle;
    ioctl(pDisp->fdDisp, DISPIO_SET_PRI_ENABLE, 1);	
	ioctl(pDisp->fdDisp, DISPIO_SET_OSD_ENABLE(0), 0);
	ioctl(pDisp->fdDisp, DISPIO_SET_OSD_ENABLE(1), 0);
}
...
void drawLine(unsigned short* drawImg, int startx, int starty, int endx, int endy, int colorR, int colorG, int colorB)
{
	...
	*/
	...
	for(t=0; t<=distance+1; t++)
	{
		...
		{
			if(startx > 0 && startx < RESOLUTION_WIDTH && starty > 0 && starty < RESOLUTION_HEIGHT)
				drawImg[startx + starty*RESOLUTION_WIDTH] = colorRGB565;			
		}
		xerr+=delta_x;
		yerr+=delta_y;
		if(xerr>distance) {
	...
	else
	{
		...
		if(dv_set.display_mode == SP_DISP_OUTPUT_LCD)
		{
			...
			else//16:9
			{
				x_offset = 46;
#if CVR_DUAL
				y_fixed = 166;//660*272[height]/1080;	
#else
		...
		ldw_displine.LBP_X = widthdest/2;
		ldw_displine.LTP_X = widthdest/2;
		ldw_displine.LTP_Y = y_fixed;//660*405[height]/1080;					
		ldw_displine.LBP_Y = y_fixed+11;
...
SINT32 gp_Icon_draw(void *destbuf,int x_dest, int y_dest,int widthdest, int heightdest,UINT8 icon_index,UINT16 background_color)
{
	...
	if(addr != NULL)
	{
		//printf("ICON index = %d X=%d,Y=%d\n",icon_index,x_dest,y_dest);
...
void ap_setting_date_time_string_process(UINT8 dt_tag,UINT8 type)
{
	...
	if (dt_tag == SETTING_DATE_TIME_DRAW_ALL) {
		...
		else if(type == 1)
		{
			...
			*(dt_str1 + tag1[(0<<1)]) = data_tmp + 0x30;
			data_tmp = (setup_date_time[1] - (10*data_tmp));
			*(dt_str1 + tag1[(0<<1)+1]) = data_tmp + 0x30;
			data_tmp = setup_date_time[2]/10;
			*(dt_str1 + tag1[(1<<1)]) = data_tmp + 0x30;
			data_tmp = (setup_date_time[2] - (10*data_tmp));
			*(dt_str1 + tag1[(1<<1)+1]) = data_tmp + 0x30;	
			data_tmp = setup_date_time[0]/10;
			*(dt_str1 + tag1[(2<<1)]) = data_tmp + 0x30;
			data_tmp = (setup_date_time[0] - (10*data_tmp));
			*(dt_str1 + tag1[(2<<1)+1]) = data_tmp + 0x30;
			for(i=3;i<6;i++)
			{
				data_tmp = setup_date_time[i]/10;
				*(dt_str1 + tag1[(i<<1)]) = data_tmp + 0x30;
				data_tmp = (setup_date_time[i] - (10*data_tmp));
				*(dt_str1 + tag1[(i<<1)+1]) = data_tmp + 0x30;				
			}

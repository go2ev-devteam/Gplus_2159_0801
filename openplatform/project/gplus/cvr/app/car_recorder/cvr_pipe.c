...
//	printf("@@  pCmdPacket->infoID %d\n",pCmdPacket->infoID);
	if((DV_CMD_MIN < pCmdPacket->infoID) && (pCmdPacket->infoID < DV_CMD_MAX)){
		...
		else if(pCmdPacket->infoID == CMD_SET_LDW_LOW_SPEED)
		{
			...
			#if LDW_TEST
			#if ADAS_CALIBRATE
			if(ap_state_config_LDW_Sound_get()==1 && ap_state_config_ADAS_sound_get()==1)
			#else
		...
		{
			if((DV_config_get(DV_LDW)&0x7F)!=0)
			{
		...
		else if(pCmdPacket->infoID == CMD_SET_LDW_INT)
		{
			...
			if((DV_config_get(DV_LDW)&0x7F)!=0  && ap_state_config_ADAS_sound_get())
			{
		...
		else if(pCmdPacket->infoID == CMD_GET_LDW_LINE2DISP)
		{
			//dv_set.ui_show = 1;
			//	printf("CMD_GET_LDW_LINE2DISP,show UI! %d\n",pCmdPacket->dataSize); 
			if((DV_config_get(DV_LDW)&0x7F)!=0)
			{

				ldw_get_displine.LT_alarmP_X = pCmdPacket->data[0]*256+pCmdPacket->data[1];
				ldw_get_displine.LT_alarmP_Y = pCmdPacket->data[2]*256+pCmdPacket->data[3];
				ldw_get_displine.LB_alarmP_X = pCmdPacket->data[4]*256+pCmdPacket->data[5];
				ldw_get_displine.LB_alarmP_Y = pCmdPacket->data[6]*256+pCmdPacket->data[7];
				
				ldw_get_displine.RT_alarmP_X = pCmdPacket->data[8]*256+pCmdPacket->data[9];
				ldw_get_displine.RT_alarmP_Y = pCmdPacket->data[10]*256+pCmdPacket->data[11];
				ldw_get_displine.RB_alarmP_X = pCmdPacket->data[12]*256+pCmdPacket->data[13];
				ldw_get_displine.RB_alarmP_Y = pCmdPacket->data[14]*256+pCmdPacket->data[15];
				

			
				ldw_get_displine.LTP_X = pCmdPacket->data[16]*256+pCmdPacket->data[17];
				ldw_get_displine.LTP_Y = pCmdPacket->data[18]*256+pCmdPacket->data[19];
				ldw_get_displine.LBP_X = pCmdPacket->data[20]*256+pCmdPacket->data[21];
				ldw_get_displine.LBP_Y = pCmdPacket->data[22]*256+pCmdPacket->data[23];

				ldw_get_displine.RTP_X = pCmdPacket->data[24]*256+pCmdPacket->data[25];
				ldw_get_displine.RTP_Y = pCmdPacket->data[26]*256+pCmdPacket->data[27];
				ldw_get_displine.RBP_X = pCmdPacket->data[28]*256+pCmdPacket->data[29];
				ldw_get_displine.RBP_Y = pCmdPacket->data[30]*256+pCmdPacket->data[31];

				ldw_get_displine.LLAlarmFlg = pCmdPacket->data[32];
				ldw_get_displine.LLcheckFlg = pCmdPacket->data[33];
				ldw_get_displine.RLAlarmFlg = pCmdPacket->data[34];
				ldw_get_displine.RLcheckFlg = pCmdPacket->data[35];	
				
				msg_id = dv_set.dv_UI_flag; 
				mq_send(menumodeMsgQ, &msg_id, 1, 0);		
			}
		}
		else if(pCmdPacket->infoID == CMD_SET_LDW_FCW)
		{
			//dv_set.LDW_INT_flag = 1;
			if(DV_config_get(DV_FCW)!=0 && ap_state_config_ADAS_sound_get())			
			{
				msg_id = SOUND_LDW_FCW;
				mq_send(KeySoundMsgQ, &msg_id, 1, 0);		
				printf("callback:LDW FCW !\n"); 			
			}
		}	
		else if(pCmdPacket->infoID == CMD_SET_LDW_STOPALRAM)
		{
			if(DV_config_get(DV_STOPANDGO)!=0 && ap_state_config_ADAS_sound_get())
			{
				//dv_set.LDW_INT_flag = 1;
				msg_id = SOUND_LDW_STOPALRAM;
				mq_send(KeySoundMsgQ, &msg_id, 1, 0);		
				printf("callback:LDW STOPALARM !\n"); 
			}
		}	
		else if(pCmdPacket->infoID == CMD_SET_LDW_GOALRAM)
		{
			if(DV_config_get(DV_STOPANDGO)!=0&& ap_state_config_ADAS_sound_get())
			{
				//dv_set.LDW_INT_flag = 1;
				msg_id = SOUND_LDW_GOALRAM;
				mq_send(KeySoundMsgQ, &msg_id, 1, 0);		
				printf("callback:LDW GOALARM !\n"); 
			}
		}	
		else if(pCmdPacket->infoID == CMD_SET_LDW_FCW_FLAG)
		{
			if(DV_config_get(DV_FCW)!=0)
			{
				dv_set.LDW_FCW_flag= pCmdPacket->data[0];
				printf("callback:SetLDW FCW flag%d !\n",dv_set.LDW_FCW_flag); 
			}				
		}
		else if(pCmdPacket->infoID == CMD_SET_LDW_STOPandGOALRAM_FLAG)
		{
			if(DV_config_get(DV_STOPANDGO)!=0)
			{
				dv_set.LDW_StopAndGo_flag= pCmdPacket->data[0];
				printf("callback:SetLDW stopandgo flag%d !\n",dv_set.LDW_StopAndGo_flag); 
			}
		}
...
static int dv_parameter(char** argv)
{
	...
	if (i > 0)
	{
		...
		argv[idx] = strdup("0");
		sprintf(argv[idx++], "%d", ap_state_config_LDW_SFCW_get());
		argv[idx] = strdup("0");
		sprintf(argv[idx++], "%d", ap_state_config_LDW_STOPAndGO_get());
	}
...
#if CVR_PLATE_NUMBER
void DV_set_plate_number(char *Number)
{
	UINT32 datasize;
	publicPlayerCmdPacket_t *packet;
	printf("@@@@@@@@ Set plate number [%s]\n", Number);
	datasize = sizeof(publicPlayerCmdPacket_t)+strlen(Number)+1;
	packet =(publicPlayerCmdPacket_t *) malloc(datasize*sizeof(char));
	packet->infoID = CMD_SET_PLATE_NUMBER;
	packet->dataSize = strlen(Number)+1;
	strcpy((char *)((char *)packet + sizeof(publicPlayerCmdPacket_t) + 0), Number);
	printf("@@@@@@@@ Set plate number [%s]\n", (char *)((char *)packet + sizeof(publicPlayerCmdPacket_t) + 0));
	while(dv_set.pipe_lock == 0)
	{
		usleep(10);
...
void CVR_SET_LDW(void)
{
	...
	LDW_options.LDW_SFCW = ap_state_config_LDW_SFCW_get();
	LDW_options.LDW_StopAndGo = ap_state_config_LDW_STOPAndGO_get();
	printf("LDW_options.EN = %d\n",LDW_options.LDW_EN);
	printf("LDW_options.cartype = %d\n",ap_state_config_LDW_Car_type_get());
	...
	printf("LDW_options.LDW_SFCW = %d\n",LDW_options.LDW_SFCW);
	printf("LDW_options.LDW_STOPAndGO = %d\n",LDW_options.LDW_StopAndGo);

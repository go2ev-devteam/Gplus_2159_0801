#ifndef __AP_STATE_CONFIG_H__
#define __AP_STATE_CONFIG_H__
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
...
//32   
	UINT8 	date_format;
	UINT8	gsensor1;
	UINT8	LDW;//LDW&0x80==0x80 used as "FCW Or stop&go" opened Flag--szk
	UINT8	LDW_Sensitivity;
	UINT8	LDW_Area_Choice;
	UINT8	LDW_Car_type;
	UINT8	LDW_speed;
...
//90
	...
	UINT8 LDW_stopandgo;
//94
	UINT8 Gsensor_initoffset[6];//record Gsensor original offset --different gsensor have different value
//100	
	UINT8 Gsensor_zoff_poweroff;//record power gzoff,if power On,gz0ff not equal Gsensor_zoff_poweroff+-1,then adas calibrate
//101
	UINT8 ADAS_sound;
} USER_ITEMS;
...
typedef struct dv_set_s{
	...
	SINT8 item_select_count2;//ADAS --LDW--LDWONOFF--ON/Off
	UINT8 sd_check;
	UINT8 battery_state;
	UINT8 dv_ctrl;
	UINT8 dc_ctrl;
...
/*volatile*/ UINT8 zoom_flag;
	...
	short GSensor_initOffset[3];
	UINT8 GSensor_calibrate_iconflag;//1:if  gsensor not  calibrate,"G"icon disp with clolor green
	UINT8 GSensor_calibrate_flag;//0: calibrate ok
								 //1:calibrate warning 
								 
								 //2keep static£¬vertical ,--------------then press ok    
								 //3waiting gsensor ....
								 //4gsensor ok 

								 //5,choose car type
								 //6up and down to disappear Arrow 
								 //7adas calibrateok,please press ok   --------then press ok
								 //8display ADAS calibrate OK
	SINT8 GSensor_calibrate_cartype_choice;//
	UINT8 ADAS_SoundOnOff_dispString;//0 not disp 1:disp Adas sound On 2:disp Adas sound Off

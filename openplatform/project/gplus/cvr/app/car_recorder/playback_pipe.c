...
static int g_width, g_height, g_disp;
	...
	if((PLAYBACK_CMD_MIN < pCmdPacket->infoID) && (pCmdPacket->infoID < PLAYBACK_CMD_MAX)){
		switch(pCmdPacket->infoID) {
			...
			case CMD_GET_FILE_DUAL_IDX:
				g_cdvr->fileStat = *(int *)pData;
				printf("%s:%d file stat: %d\n", __FUNCTION__, __LINE__, g_cdvr->fileStat);
				break;
			default:
				break;
		}
...
cdvr_thmb_t *playback_reboot(void)
{
	return playback_init(g_disp, g_width, g_height);	
}

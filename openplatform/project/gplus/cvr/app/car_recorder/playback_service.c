...
void PLAYBACK_UI_foreground(void)
{
	...
	//	if(dispMode == DISP_FULL_SCREEN)
		...
		//sprintf(buf, "%d/%d", dvr_thmb_getCurIdxOfFile(pdvr)+1, pdvr->file_number);
#if CVR_DUAL
		sprintf(buf, "%d/%d  %s", dvr_thmb_getCurIdxOfFile(pdvr)+1, pdvr->file_number, FilelistGetFileName(dvr_thmb_getCurIdxOfFile(pdvr)));
#else
		sprintf(buf, "%s", FilelistGetFileName(dvr_thmb_getCurIdxOfFile(pdvr)));
#endif
		char *pExt = strchr(buf, '.');
...
#if 0//CVR_DUAL
		ascii_str.pos_x = resolution.width + (STR_FILENUM_POX_X)*HDMIxNum;
		ascii_str.pos_y = resolution.height - (STR_FILENUM_POX_Y-3)*HDMIxNum;//- ICON_LEFT_Y_POS*2;
		/*if(pdvr->fileStat == 1) {
...
/**
 * for video to set TV NTSC or PAL.
 **/
int dvr_thmb_setTVMode(int mode)
{
	int ret;
	//ret = playback_Set_PIPE(CMD_SET_TV_MODE, &mode, sizeof(mode));
	playback_uninit(pdvr);
	pdvr = NULL;
	pdvr = playback_reboot();
	return 0;
}

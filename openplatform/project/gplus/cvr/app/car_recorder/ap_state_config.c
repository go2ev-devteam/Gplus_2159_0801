...
#define SDCARD_LABEL_ENABLE 0
#define gp_Label "GENERALPLUS"
...
static void ap_state_config_LDW_set(UINT8 type)
{
	if (type != ap_state_config_LDW_get()) {
		if(1==type) 
		{
			Global_User_Optins.item.LDW |= 0x01;
		}else
		{
			Global_User_Optins.item.LDW &= ~0x01;
		}
...
UINT8 ap_state_config_LDW_SFCW_get(void)
{
	return Global_User_Optins.item.LDW_sfcw;
}
UINT8 ap_state_config_LDW_STOPAndGO_get(void)
{
	return Global_User_Optins.item.LDW_stopandgo;
}

void ap_state_config_LDW_SFCW_set(UINT8 type)
{
	if (type != ap_state_config_LDW_SFCW_get()) 
	{
		if(1==type)
		{
			Global_User_Optins.item.LDW |= 0x80;			
		}else if(0==type)
		{
			if(0 == ap_state_config_LDW_STOPAndGO_get())
			{
				Global_User_Optins.item.LDW &= ~0x80;		
			}
		}
		Global_User_Optins.item.LDW_sfcw= type;
		LDW_Change = 1;
		Global_User_Optins.item.ifdirty = 1;
	}
	adas_warning_flag=0;
}
void ap_state_config_LDW_STOPAndGO_set(UINT8 type)
{
	if (type != ap_state_config_LDW_STOPAndGO_get()) {
		if(1==type)
		{
			Global_User_Optins.item.LDW |= 0x80;			
		}else if(0==type)
		{
			if(0 == ap_state_config_LDW_SFCW_get())
			{
				Global_User_Optins.item.LDW &= ~0x80;			
			}
		}
		Global_User_Optins.item.LDW_stopandgo= type;
		LDW_Change = 1;
		Global_User_Optins.item.ifdirty = 1;
	}
	adas_warning_flag = 0;
...
UINT8 ap_state_config_ADAS_sound_get(void)
{
	return Global_User_Optins.item.ADAS_sound;
}

void ap_state_config_ADAS_sound_set(UINT8 type)
{
	if (type != ap_state_config_ADAS_sound_get()) {
		Global_User_Optins.item.ADAS_sound = type;
		Global_User_Optins.item.ifdirty = 1;
	}
}
...
UINT8 DV_config_get(SINT8 index)
{
	...
	*/
	...
	{	
		...
		case DV_FCW:
			ret = ap_state_config_LDW_SFCW_get();
		break;	

		case DV_STOPANDGO:
			ret = ap_state_config_LDW_STOPAndGO_get();
		break;	
...
UINT8 DV_config_set(UINT8 index,UINT8 data)
{
	...
	{	
		...
		break;		
		case DV_FCW:
			ap_state_config_LDW_SFCW_set(data);
		break;	
		case DV_STOPANDGO:
			ap_state_config_LDW_STOPAndGO_set(data);
		break;	
		case DV_TIMELAPSE:
...
#if CVR_PLATE_NUMBER
void Dv_plate_number_init(void)
{
	int i;
	for(int i=0;i<8;i++)
	{
		printf("%d", Global_User_Optins.item.carnum[2*i]);
		if(Global_User_Optins.item.carnum[2*i] == 0)
		{
			Global_User_Optins.item.carnum[2*i] = 'A';
		}
		if(Global_User_Optins.item.carnum[2*i+1] == 0)
		{
			if(i != 7)
				Global_User_Optins.item.carnum[2*i+1]=' ';
			else
				Global_User_Optins.item.carnum[2*i+1]= 0;	
		}
	}
}
#endif
...
int Sdcard_check_Label(char *pLabel)
{
	int ret = -1;
	char buf[256] = {0};
	char *ptr = NULL;
	...
	ret = fread( buf, sizeof(unsigned char), sizeof(buf), fp);
	pclose(fp);

	ptr = strstr(buf, pLabel);
	if(NULL == ptr) {
		return -1;
	}
...
static int Format(char* mount_name )
{
	...
	/*format*/
	memset(buffer, 0, sizeof(buffer));
#if SDCARD_LABEL_ENABLE
	if (fat_type == 1)
		sprintf(buffer,"mkfs.fat -n %s -F 16 -s 64 \"%s\"", gp_Label, dev_name);
	else if (fat_type == 2)
		sprintf(buffer,"mkfs.fat -n %s -F 32 -s 64 \"%s\"", gp_Label, dev_name);
	else if (fat_type == 3)
	//	sprintf(buffer,"mkfs.exfat \"%s\"", dev_name);
		sprintf(buffer,"mkfs.fat -n %s -F 32 -s 128 \"%s\"", gp_Label, dev_name);
#else
	if (fat_type == 1)
...
#if SDCARD_LABEL_ENABLE
	if (fat_type == 1)
		sprintf(buffer,"mkfs.fat -n %s -F 16 -s 64 \"%s\"", dev_name);
	else if (fat_type == 2)
		sprintf(buffer,"mkfs.fat -n %s -F 32 -s 64 \"%s\"", dev_name);
	else if (fat_type == 3)
	//	sprintf(buffer,"mkfs.exfat \"%s\"", dev_name);
		sprintf(buffer,"mkfs.fat -n %s -F 32 -s 128 \"%s\"", dev_name);
#else
	if (fat_type == 1)
		sprintf(buffer,"mkfs.fat -F 16 -s 64 \"%s\"", dev_name);
	else if (fat_type == 2)
...
int check_cluster_size() {
	...
	stat(Memory_path, &fstat);
	cluster_size = fstat.st_blksize;
...
#if SDCARD_LABEL_ENABLE
		if(Sdcard_check_Label(gp_Label)!=0) {
			dv_set.sdc_error= 1;
			time_num = 2;
			printf("LABEL ERROR! please format!\r\n");
		}
		else 
#endif
		{
...
SINT32 SPI_Flash_update(void)
{
	...
	else if(file_size > (4*1024*1024-4*1024))
	{
		UPgrade_UI(7,0);
		UPgrade_UI(7,0);
		close(sdc_fd);
	...
	for(i=0;i<loop_num;i++)
	{
		...
		ioctl(spi_fd, SPI_IOCTL_CS_DISABLE, 0);	
		if((file_size%256!=0)&&(i == (loop_num -1) ))
		check_num = file_size%256;
		else
		check_num = 256;
		for(j=0;j<256;j++)
		{
			if(pInput[i*256+j] != buffer1[j])
			{
				close(spi_fd);
				gpChunkMemFree(pInput);		
				return -1;
...
#define ASENSOR_IOCTL_SET_INIT_OFFSET	_IOW(ASENSOR_IOCTL_ID, 0x20, int)
#define ASENSOR_IOCTL_GET_INIT_OFFSET	_IOR(ASENSOR_IOCTL_ID, 0x40, int)
#define ASENSOR_IOCTL_CALIBRATE_ONOFF	    _IOW(ASENSOR_IOCTL_ID, 0x80, int)
#define ASENSOR_IOCTL_GET_CALIBRATE_OFFSET	_IOW(ASENSOR_IOCTL_ID, 0x81, int)
#define ASENSOR_IOCTL_CALIBRATE_ENABLE		_IOW(ASENSOR_IOCTL_ID, 0x82, int)

#if ADAS_CALIBRATE

//将spi保存的gsensor的初始偏移设置进gsensor的driver中
char  Gsensor_Set_initoffset(short *gsensor_initoffset)
{
	int fd;
	...
	printf("Gsensor_set_inioffset %d,%d,%d\n\n\n\n",gsensor_initoffset[0],gsensor_initoffset[1],gsensor_initoffset[2]);
	ioctl(fd, ASENSOR_IOCTL_SET_INIT_OFFSET, gsensor_initoffset);
	close(fd);
	return 1;
}

//获取gsensor的初始偏移，之后会保存到spi
char  Gsensor_Get_initoffset(short *gsensor_initoffset)
{
	int fd;
	fd = open("/dev/gp_asensor",O_RDWR);
	if(fd<0){		
	...
	ioctl(fd, ASENSOR_IOCTL_GET_INIT_OFFSET, gsensor_initoffset);
	printf("Gsensor_Get_initoffset  %d,%d,%d\n",gsensor_initoffset[0],gsensor_initoffset[1],gsensor_initoffset[2]);
	close(fd);
	return 1;
}
void setting_get_gsensor_initoffset(short *gsensor_initoffset)
{	
	gsensor_initoffset[0]=Global_User_Optins.item.Gsensor_initoffset[0]*256+Global_User_Optins.item.Gsensor_initoffset[1];	
	gsensor_initoffset[1]=Global_User_Optins.item.Gsensor_initoffset[2]*256+Global_User_Optins.item.Gsensor_initoffset[3];	
	gsensor_initoffset[2]=Global_User_Optins.item.Gsensor_initoffset[4]*256+Global_User_Optins.item.Gsensor_initoffset[5];	
}
void setting_set_gsensor_initoffset(short *gsensor_initoffset)
{
	Global_User_Optins.item.Gsensor_initoffset[0]=(gsensor_initoffset[0]&0xff00)>>8;
	Global_User_Optins.item.Gsensor_initoffset[1]=(gsensor_initoffset[0]&0x00ff);
	Global_User_Optins.item.Gsensor_initoffset[2]=(gsensor_initoffset[1]&0xff00)>>8;
	Global_User_Optins.item.Gsensor_initoffset[3]=(gsensor_initoffset[1]&0x00ff);
	Global_User_Optins.item.Gsensor_initoffset[4]=(gsensor_initoffset[2]&0xff00)>>8;
	Global_User_Optins.item.Gsensor_initoffset[5]=(gsensor_initoffset[2]&0x00ff);
	Global_User_Optins.item.ifdirty = 1;
}
//使能gsensor calibrate，enable后，gsensor传给ldw的gvalue=0.  on = 1:enable
char  Gsensor_Calibrate_enable(int on)
{
	int fd;
	fd = open("/dev/gp_asensor",O_RDWR);
	if(fd<0){		
	...
	ioctl(fd, ASENSOR_IOCTL_CALIBRATE_ENABLE, &on);
	close(fd);
	return 1;
}
//gsensor 进入calibrate状态，这时ldw获取的gvalue=0，timer中会读取10次gsensor的数据，然后去掉最大值和最小值，剩余的8个数据取平均值。
//再通过Gsensor_Calibrate_read_z_offset获取平均值与期望角度的差值，作为箭头显示长短的参量
//on =0 ,off   on = 1:car view angle ,start calibrate  on=2 suv view angle  4 truck view angle
char  Gsensor_Calibrate_ONOFF(int on)
{
	int fd;
	fd = open("/dev/gp_asensor",O_RDWR);
	if(fd<0){		
	...
	ioctl(fd, ASENSOR_IOCTL_CALIBRATE_ONOFF, &on);
	close(fd);
	return 1;
}
//获取zvalue 相对于期望角度的差值，作为箭头长短的指示
char  Gsensor_Calibrate_read_z_offset(short *z_off)
{
	int fd;
	fd = open("/dev/gp_asensor",O_RDWR);
	if(fd<0){		
	...
	ioctl(fd, ASENSOR_IOCTL_GET_CALIBRATE_OFFSET, z_off);
	close(fd);
	return 1;
}
//保存关机时的zoff。开机后如果在合理范围内，zoff偏差正负1，则不需要进入calibrate界面
void setting_set_gsensor_gzoff_poweroff(short *zoff_poweroff)
{
	Global_User_Optins.item.Gsensor_zoff_poweroff = *zoff_poweroff+128;
	printf("pweroff-test %d\n",Global_User_Optins.item.Gsensor_zoff_poweroff);
	Global_User_Optins.item.ifdirty = 1;
}
void setting_get_gsensor_gzoff_poweroff(short *zoff_poweroff)
{
	printf("pweroff-test -ON %d\n",Global_User_Optins.item.Gsensor_zoff_poweroff);

	*zoff_poweroff = Global_User_Optins.item.Gsensor_zoff_poweroff-128;
}


#endif

char  Gsensor_Set_Int_Level(int level)
{
	int fd;
	int temp;
	fd = open("/dev/gp_asensor",O_RDWR);
	...
	ioctl(fd, ASENSOR_IOCTL_SET_INIT_LEVEL, level);
	close(fd);
	return temp;
}

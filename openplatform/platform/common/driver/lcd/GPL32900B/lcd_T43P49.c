...
static panel_lcdInfo_t gPanelInfo = {
	.name = "panel_lcd_T43P49", // MUST equal module name
	.workFreq    = 27000000,
	.clkPolatiry = 1,
	.resolution = {
		.width  = 480,
	...
	.type        = PANEL_TYPE_SRGB888,
	.dataSeqEven = PANEL_SEQUENCE_SRGB888_RGB,
	.dataSeqOdd  = PANEL_SEQUENCE_SRGB888_RGB,
	.vsync = {
		.polarity = 0,
		.fPorch   = 3,
		.bPorch   = 12,
...
)
{
	...
	return panel_common_init(GID_SRGB888);
}
...
{
	DEBUG("[%s:%d]\n", __FUNCTION__, __LINE__);
	return panel_common_suspend(GID_SRGB888);
}
...
{
	DEBUG("[%s:%d]\n", __FUNCTION__, __LINE__);
	return panel_common_resume(GID_SRGB888);
}

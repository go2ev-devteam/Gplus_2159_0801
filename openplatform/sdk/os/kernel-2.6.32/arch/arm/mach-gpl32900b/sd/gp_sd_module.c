...
#define GP_TAG						0x4D4D4453	/* TAG: SDMM */
#define APP_TAG						0x50415047	/* TAG: GPAP */
...
#define gp_sdcard_inner_card(x) (x->sd_func->detect ? 0 : 1 ) /* If there is no card detection function defined, it is an inner card */
...
extern int gp_sdcard_get_status(gpSDInfo_t * sd);
extern int gp_sdcard_cardinit (gpSDInfo_t* sd);
extern int gp_sdcard_cardreinit (gpSDInfo_t* sd);
...
extern int gp_sdcard_readcmd(unsigned int device_id, unsigned int sector);
extern int gp_sdcard_writecmd(unsigned int device_id, unsigned int sector);
extern int gp_sdcard_dma_en(unsigned int device_id, unsigned char* buf, unsigned int ln, unsigned int dir);
extern int gp_sdcard_dma_finish(unsigned int device_id, unsigned short timeout);
extern int gp_sdcard_stop(unsigned int device_id);
extern int gp_sdcard_dma_stop(struct block_device *dev);
extern SP_BOOL gpHalSDWaitDataComplete(UINT32 device_id);
/**************************************************************************
*                         G L O B A L    D A T A                         *
**************************************************************************/
...
unsigned short limit_clock = 500;//250;
module_param_named(limit_clock, limit_clock, ushort, S_IRUGO);

gpSDInfo_t *sd_info = NULL;

/**
 * @brief   SD Class OPS, for SD Driver internal reference
 */
...
/**
 * @brief   SD platform device define
 */
...
/**
 * @brief   SD platform device driver define
 */
...
/**
* @brief 	Check card write protect function.
* @param 	sd[in]: Card information.
* @return 	0 for remove, 1 for insert.
*/
static int gp_sdcard_ckwp(gpSDInfo_t* sd)
{
	//if(gp_sdcard_get_status(sd)<0)
	//	return 1;
	if(sd->sd_func->is_write_protected==NULL)
		return 0;
	return (sd->sd_func->is_write_protected()==1)?1:0;
}
...
/**
 * @brief   sd device release function
 */
static void gp_sd_device_release(struct device *dev)
{
...
EXPORT_SYMBOL(gp_sdcard_app_read_sector);
...
*/
unsigned int  gp_sdcard_get_sn(void)
{
	int i;
	...
	for(i = 0; i < SD_NUM; i++)
	{
		...
		if( sd->present && sd->partition.activity )
			return sd->partition.sn ;
	}
	return 0;
}
EXPORT_SYMBOL(gp_sdcard_get_sn);

...
static const  gpHalSDCmd_t SD_ACMD23	= {23, 	SDC_RESPTYPE_R1, 	SDC_NO_DATA, 		0};
...
static const  gpHalSDCmd_t MMC_CMD14	= {14, 	SDC_RESPTYPE_R1, 	SDC_SINGLE_DATA, 	SDC_READ};
static const  gpHalSDCmd_t MMC_CMD19	= {19, 	SDC_RESPTYPE_R1, 	SDC_SINGLE_DATA, 	SDC_WRITE};
...
#define gp_sdcard_ch_resp(resp) (resp&0xfff98008 ? 1:0)
...
extern unsigned short limit_clock;
...
{
	unsigned char SpeedTable[16]={0, 10, 12, 13, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 70, 80};		/* mmc table is diffent at element 11: 50 ->52 */
	unsigned short TranTable[4]={1, 10, 100, 1000};
	unsigned short speed = SpeedTable[(factor& 0x78)>>3] * TranTable[factor & 0x07] / 10;
	return (speed>limit_clock)? limit_clock : speed;
}
...
* @return 	SUCCESS/ERROR_ID.
*/
int gp_sdcard_readcmd(unsigned int device_id, unsigned int sector)
{
	unsigned int response = 0;
	if(device_id>=SD_NUM)
		return SP_FALSE;
	...
	/* ----- Check response error bit ----- */
	if(unlikely(gp_sdcard_ch_resp(response)))
	{
		DERROR("[%d]: resp = %d error\n", response );
		return SP_FALSE;
	}
...
* @return 	SUCCESS/ERROR_ID.
 */
int gp_sdcard_writecmd(unsigned int device_id, unsigned int sector)
{
	unsigned int response = 0;
	if(device_id>=SD_NUM)
		return SP_FALSE;
...
* @return 	SUCCESS/ERROR_ID.
 */
int gp_sdcard_dma_en(unsigned int device_id, unsigned char* buf, unsigned int ln, unsigned int dir)
{
#ifdef USE_DMA
...
* @return 	SUCCESS/ERROR_ID.
 */
int gp_sdcard_dma_finish(unsigned int device_id, unsigned short timeout)
{
#ifdef USE_DMA
#ifndef POLL
...
* @return 	SUCCESS/ERROR_ID.
 */
int gp_sdcard_stop(unsigned int device_id)
{
	unsigned int response = 0;
...
/**
* @brief 	Set MMC card bus width function.
* @param 	sd[in]: Card information.
* @param	bus_width[in]: 0 for 1 bit mode, 1 for 4 bits mode..
* @return 	SUCCESS/ERROR_ID.
*/
static bool gp_sdcard_mmc_set_bus_width(gpSDInfo_t* sd, unsigned int bus_width)
{
	unsigned int response = 0;
	unsigned int now_bus_width = gpHalSDGetBus(sd->device_id);
	unsigned int test_pattern;
	unsigned int receive_pattern = 0;

	if(now_bus_width == bus_width)
		return SP_TRUE;

	gpHalSDSetBus( sd->device_id, bus_width);

	if(bus_width)
	{
		test_pattern = 0x5A;
		gpHalSDSetBlk(sd->device_id, 4);
		/* ----- CMD19 to send test pattern for bus testing ----- */
		if(gpHalSDSendCmd(sd->device_id, MMC_CMD19, 0x00, &response, 4)==SP_FALSE)
			goto out_err;
		gpHalSDDataTxRx(sd->device_id, (UINT8*)&test_pattern, 4, 1);
		/* ----- CMD14 to receive test pattern for bus testing  ----- */
		if(gpHalSDSendCmd(sd->device_id, MMC_CMD14, 0x00, &response, 4)==SP_FALSE)
			goto out_err;
		gpHalSDDataTxRx(sd->device_id, (UINT8*)&receive_pattern, 4, 0);
		/* ----- Check test pattern ----- */
		if((test_pattern^receive_pattern)!=0xFF)
			goto out_err;
	...
	else
	{
		...
		/* ----- Check test pattern ----- */
		receive_pattern >>= 24;
		if((test_pattern^receive_pattern)!=0xC0)
			goto out_err;
		/* ----- Setup MMC bus width 1 bit----- */
		if(gpHalSDSendCmd(sd->device_id, MMC_CMD6, 0x03b70000, &response, 4) == SP_FALSE)
			goto out_err;
		if(response&0x08)
			goto out_err;
	}
...
out_err:
	gpHalSDSetBlk(sd->device_id, 512);
	gpHalSDSetBus( sd->device_id, now_bus_width);
	return SP_FALSE;
}
...
int gp_sdcard_setbus(gpSDInfo_t* sd, unsigned int mode)
{
	...
	{
		if(gp_sdcard_mmc_set_bus_width(sd, mode)==SP_FALSE)
			return -EIO;
	}
	else
		return -EIO;
...
static bool gp_sdcard_mmc_setup(gpSDInfo_t* sd, unsigned short sys_apb)
{
	...
	if(SPEC_VAR>=4)
	{
		...
		/* ----- Setup MMC bus width 4 bit----- */
		if(gp_sdcard_mmc_set_bus_width( sd, 1 )==SP_FALSE)
			DERROR("[%d]: Switch 4 bit mode fail\n", sd->device_id);
	}
	/* ----- Calculate Totoal Memory Size ----- */
...
* @return 	SUCCESS/ERROR_ID.
*/
int gp_sdcard_cardreinit (gpSDInfo_t* sd)
{
	unsigned long sys_apb_clk = 0;
	struct clk *pclk;
...
/**Added for USB gadget by Leo**/
int gp_sdcard_dma_stop(struct block_device *dev)
{

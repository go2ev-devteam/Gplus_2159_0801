...
const unsigned char g_nd_edge_table[256] = // add by Comi , not strong edge
{
	0x00, 0x01, 0x02, 0x04, 0x07, 0x0a, 0x0e, 0x12, 0x17, 0x1d, 0x23, 0x29, 0x31, 0x38, 0x41, 0x4a,
...
*/
		...
		if(argp->ndeluten) {
			if(argp->ndedge_table) {
				...
				//gpHalCdspInitEdgeLut((unsigned char *)g_edge_table);
				gpHalCdspInitEdgeLut((unsigned char *)g_nd_edge_table);
			}
		}
...
{
	gpHalCdspSetAEAWBEn(argp->ae_en, argp->awb_en);
}
...
)
{
	...
	switch(ctrl->id)
	{
		...
		case MSG_CDSP_AE_AWB_CTRL:
		{
			gpCdspAEAWB_t aeawb_ctrl;

			nRet = copy_from_user((void*)&aeawb_ctrl, (void __user*)ctrl->value, sizeof(aeawb_ctrl));
			if(nRet < 0) {
				RETURN(-EINVAL);
			}

			gpCdspSetAEAWB(&aeawb_ctrl);
			break;
		}
...
)
{
	...
	switch(cmd)
	{
		...
		case VIDIOC_S_CALIBRATE:
		{
			...
			CdspDev->sensor_cdsp.awb_size = calibrate.awb_size;
...
/*
				...
				/*if(idx == 3) gpHalCdspSetYuvHAvg(3, 2, 2, 2);
				else if(idx == 2) gpHalCdspSetYuvHAvg(3, 1, 2, 2);
				else if(idx == 1) gpHalCdspSetYuvHAvg(3, 1, 1, 1);
				else	gpHalCdspSetYuvHAvg(3, 0, 0, 0);*/
	...
	if(1) { //(status & FRONT_VD_INT_BIT)
		UINT32 irq_status, irq_en;
		
		gpHalFrontIntGet(&irq_en);
		gpHalFrontIntEventGet(&irq_status);
		gpHalFrontIntEventClr(irq_status);
		irq_status &= irq_en;
		
		//printk("FRONT_VD_INT_BIT = 0x%x\n", status);
		if(irq_status & 0x20) {
			// reset hd count
			gpHalFrontHdCntSet(0x8000);
	...
	while(1) {
		...
		{		
			...
			if(awb_frame >= 2 && (CdspDev->ae_awb_flag & CDSP_AWB_SET_GAIN) == 0 && lum > targetY_low)
			{
				...
				if(awb_ret == AWB_SUCCESS_CVR && awbmode == AWB_AUTO_FIRST_SEARCH && awb_init_search  == 1)
				{
					awb_uv_thr_t UVthr;

					UVthr.UL1N1 = CdspDev->sensor_cdsp.awb_thr[7];
					UVthr.UL1P1 = CdspDev->sensor_cdsp.awb_thr[8];
					UVthr.VL1N1	= CdspDev->sensor_cdsp.awb_thr[9];
					UVthr.VL1P1 = CdspDev->sensor_cdsp.awb_thr[10];

					UVthr.UL1N2 = CdspDev->sensor_cdsp.awb_thr[11];
					UVthr.UL1P2 = CdspDev->sensor_cdsp.awb_thr[12];
					UVthr.VL1N2	= CdspDev->sensor_cdsp.awb_thr[13];
					UVthr.VL1P2 = CdspDev->sensor_cdsp.awb_thr[14];

					UVthr.UL1N3 = CdspDev->sensor_cdsp.awb_thr[15];
					UVthr.UL1P3 = CdspDev->sensor_cdsp.awb_thr[16];
					UVthr.VL1N3	= CdspDev->sensor_cdsp.awb_thr[17];
					UVthr.VL1P3 = CdspDev->sensor_cdsp.awb_thr[18];
					gpHalCdspSetAwbUVThr(&UVthr);
					gp_cdsp_awb_set_mode(awb, AWB_AUTO_CVR2);
					awb_init_search = 0;
					//printk("AWB_AUTO_CVR2");
				}
				...
				if(awb_ret != AWB_RET)
				{
					...
					if(awbmode == AWB_AUTO_CVR || awbmode == AWB_AUTO_CVR_DAYLIGHT || awbmode == AWB_AUTO_CVR_NIGHT)
					{
						...
						//DEBUG("sensor_ev_idx =%d, daylight_ev_idx = %d\r\n", p_seInfo->sensor_ev_idx, p_seInfo->daylight_ev_idx);
				...
				printk("%s: r gain = %d, %d, b gain = %d, %d\r\n", __FUNCTION__, rgain, wb_gain2.rgain2,bgain,wb_gain2.bgain2);
				}*/
#endif				
				up(&CdspDev->aeawb_sem);
			}

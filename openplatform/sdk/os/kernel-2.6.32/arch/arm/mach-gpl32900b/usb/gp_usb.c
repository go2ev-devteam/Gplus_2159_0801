...
static int gp_usb_fops_open(struct inode *inode, struct file *file);
static int gp_usb_fops_write(struct file *flip, const char __user *buf, size_t count, loff_t *oppos);
...
static struct file_operations usb_device_fops = {
	.owner          = THIS_MODULE,
	.open			= gp_usb_fops_open,
	.unlocked_ioctl = gp_usb_fops_ioctl,
	.write 			= gp_usb_fops_write,
...
static int gp_usb_fops_open(struct inode *inode, struct file *flip)
{
	return 0;
}
...
static int gp_usb_fops_write(struct file *flip, const char __user *buf, size_t count, loff_t *oppos)
{
	...
	strncpy_from_user(file_buf, buf, count);
	...
	if(strncmp(file_buf, "phy1todev", 10) == 0)
		gpHalUsbPhy1toDev(0);
	else if(strncmp(file_buf, "phy1tohost", 11) == 0)
		gpHalUsbPhy1toDev(1);

...
static UINT32 ppu_fb_lock_set_flag = 0;
static UINT32 ppu_lnterlace_flag = 0;
...
static pputvFun_t *pPPUTVFunreg = (pputvFun_t *)(PPU_BASE_REG);
static pputftFun_t *pPPUTFTFunreg = (pputftFun_t *)(PPU_BASE_REG);
 /**************************************************************************
...
/**
* @brief	       ppu module mode
* @param 	enable[in]:0=disable,1=enable.
* @return 	none
*/
void gpHalPPUCtrl(UINT32 enable)
{
     if(enable)
     {
    ...
    if(!ppu_set_buffer_temp_flag)
    {
       ...
       ppu_set_buffer_temp_flag=1;
		...
		ppu_frame_mode_busy = 1;
		 ...
		 #else
		  else if(pPPUTFTFunreg->ppuTftEn & 0x1)
		 #endif
		  {
...
* @return 	success=0,fail=-1.
*/
SINT32 gpHalPPUSimpleGo(PPU_REGISTER_SETS *p_register_set)
{
	if (!p_register_set) {
		return STATUS_FAIL;
	}
	...
	pPPUFunreg->ppuFboAddr = new_register_sets_ptr->buffer_user_define;
...
EXPORT_SYMBOL(gpHalPPUSimpleGo);
...
SINT32 gpHalPPUIsr(void)
{
		...
		UINT32 temp=0;
			...
			if(!ppu_irq_temp_flag)
			{
					 ...
					 #else
						if(pPPUIrqreg->ppuIrqState & C_PPU_INT_PEND_TV_VBLANK) 
					 #endif	
						{
							  pPPUIrqreg->ppuIrqState = C_PPU_INT_PEND_TV_VBLANK; 
							  if(!ppu_irq_temp_flag)
					...
					#else
					  if(pPPUIrqreg->ppuIrqState & C_PPU_INT_PEND_TFT_VBLANK)
					#endif	
						{
							  pPPUIrqreg->ppuIrqState = C_PPU_INT_PEND_TFT_VBLANK; 
							  if(!ppu_irq_temp_flag)
		...
		#else
			if(pPPUIrqreg->ppuIrqState & C_PPU_INT_PEND_PPU_VBLANK)
		#endif
			{
				  pPPUIrqreg->ppuIrqState = C_PPU_INT_PEND_PPU_VBLANK;
			    temp = PPU_MODULE_PPU_VBLANK;	
...
void gpHalPPUFbCtrl(UINT32 enable)
{
     if(enable)
     {
        ...
        ppu_fb_lock_set_flag = 1;
     }
     else
     {
        pPPUFunreg->ppuMisc &=~FB_LOCK_ENABLE; 
        ppu_fb_lock_set_flag = 0;
     }    
}
...
)
{
	...
	if(mode > 7)
	{
		...
		pPPUFunreg->ppuEnable = regVal;	
		regVal = (mode >> 16); 
		pPPUFunreg->ppuFreesize = (((0 << B_FREE_INIT) & MASK_FREE_INIT_SIZE)|((regVal << B_FREE_H_SIZE) & MASK_FREE_H_SIZE )|(mode & MASK_FREE_V_SIZE));	
	}
	else
	{
		regVal = pPPUFunreg->ppuEnable;
...
EXPORT_SYMBOL(gpHalPPUGetFbEnable);
...
void
gpHalPPUSetVgaLnterlaceDisable(
	UINT32 enable
)
{
	UINT32 regVal;
...
EXPORT_SYMBOL(gpHalPPUSetVgaLnterlaceDisable);
void
...
)
{
	...
	regVal = pPPUTVFunreg->ppuTvEn ;
	regVal &= ~0x1;
	regVal |= enable;
	pPPUTVFunreg->ppuTvEn = regVal;
}
EXPORT_SYMBOL(gpHalPPUSetTVEnable);

void
gpHalPPUSetTFTEnable(
	UINT32 enable
)
{
	UINT32 regVal;

	regVal = pPPUTFTFunreg->ppuTftEn ;
	regVal &= ~0x1;
	regVal |= enable;
	pPPUTFTFunreg->ppuTftEn = regVal;
}
EXPORT_SYMBOL(gpHalPPUSetTFTEnable);
...
{	
#if 1   // For TV type application.
	if(enable == 0x2)
			pPPUFunreg->ppuFreesize = ((1<<B_INTEL_TYPE)|(TV_720)|(TV_576>>1));		
	else if(enable == 0x1)
			pPPUFunreg->ppuFreesize = ((1<<B_INTEL_TYPE)|(TV_640)|(TV_576>>1));		
	else if(enable == 1280)
	    pPPUFunreg->ppuFreesize = ((1<<B_INTEL_TYPE)|(TV_720P_H)|(TV_720P_V>>1));
	else
	    pPPUFunreg->ppuFreesize = 0; 
#else	  // For TFT type application.
	if(enable == 0x2)
			pPPUFunreg->ppuFreesize = ((0<<B_INTEL_TYPE)|(TV_720)|(TV_576));		
	else if(enable == 0x1)
			pPPUFunreg->ppuFreesize = ((0<<B_INTEL_TYPE)|(TV_640)|(TV_576));		
	else if(enable == 1280)
	    pPPUFunreg->ppuFreesize = ((0<<B_INTEL_TYPE)|(TV_720P_H)|(TV_720P_V));
	else
	    pPPUFunreg->ppuFreesize = 0; 
#endif	       
...
EXPORT_SYMBOL(gpHalPPUSet576mode);
...
* @return 	none
*/
void gpHalPPUFblock(PPU_FBLOCK_STRUCT *fb_lock)
{
	   if(fb_lock->fb_lock_en)
	   {
		   gpHalPPUFbCtrl(1);
		   gpHalPPUSetTVEnable(0);
		   // TV
		   switch(fb_lock->TV_color_mode)
		   {
				case PPU_COLOR_RGBG:
		   ...
		   gpHalPPUSetVgaEnable(0);
		   gpHalPPUSetRes(0);
		   gpHalPPUSetVgaLnterlaceDisable(0);
		   if((fb_lock->TV_frame_hsize == 640) && (fb_lock->TV_frame_vsize == 480))
				gpHalPPUSetVgaEnable(1);
		   else if((fb_lock->TV_frame_hsize) == 720 && (fb_lock->TV_frame_vsize == 480))
				gpHalPPUSetRes(4);
		   else
				gpHalPPUSetVgaLnterlaceDisable(1);
		   gpHalPPUSetTVEnable(1);		   
		   // TFT
		   gpHalPPUSetTFTEnable(0);
		   switch(fb_lock->TFT_color_mode)
		   {      
				case PPU_COLOR_RGBG:
				   gpHalPPUSetFbFormat(1);
				   gpHalPPUSetFbMono(0);
		   ...
		   gpHalPPUSetVgaEnable(0);
		   gpHalPPUSetVgaLnterlaceDisable(0);
		   if((fb_lock->TFT_frame_hsize == 320) && (fb_lock->TFT_frame_vsize == 240))
				gpHalPPUSetRes(0);
		   else if((fb_lock->TFT_frame_hsize) == 640 && (fb_lock->TFT_frame_vsize == 480))
				gpHalPPUSetRes(1);
		   else if((fb_lock->TFT_frame_hsize) == 480 && (fb_lock->TFT_frame_vsize == 234))
				gpHalPPUSetRes(2);
		   else if((fb_lock->TFT_frame_hsize) == 480 && (fb_lock->TFT_frame_vsize == 272))
				gpHalPPUSetRes(3);
		   else if((fb_lock->TFT_frame_hsize) == 720 && (fb_lock->TFT_frame_vsize == 480))
				gpHalPPUSetRes(4);
		   else if((fb_lock->TFT_frame_hsize) == 800 && (fb_lock->TFT_frame_vsize == 480))
				gpHalPPUSetRes(5);
		   else if((fb_lock->TFT_frame_hsize) == 800 && (fb_lock->TFT_frame_vsize == 600))
				gpHalPPUSetRes(6);
		   else if((fb_lock->TFT_frame_hsize) == 1024 && (fb_lock->TFT_frame_vsize == 768))
				gpHalPPUSetRes(7);		   
		   else
				pPPUFunreg->ppuFreesize = (((0 << B_FREE_INIT) & MASK_FREE_INIT_SIZE)|((fb_lock->TFT_frame_hsize << B_FREE_H_SIZE) & MASK_FREE_H_SIZE )|(fb_lock->TFT_frame_vsize & MASK_FREE_V_SIZE));		   
		   gpHalPPUSetTFTEnable(1);	   
	   }
	   else
		   gpHalPPUFbCtrl(0);
}
EXPORT_SYMBOL(gpHalPPUFblock);

/**
* @brief	ppu enable set
* @return 	none
*/
void
gpHalPPUEnset(
...
EXPORT_SYMBOL(gpHalPPUEnset);

/**
* @brief	ppu enable get
* @return 	none
*/
UINT32
gpHalPPUEnget(
...
{
    return (pPPUFunreg->ppuEnable & 0x1);
}
EXPORT_SYMBOL(gpHalPPUEnget);

UINT32
gpHalPPUInterlaceEnget(
	void
)
{
    UINT32 regVal = 0;
    
    if(!(pPPUFunreg->ppuEnable & 0x20)||(pPPUFunreg->ppuFreesize & C_PPU_FREESIZE_INTERLACE_ENABLE))
    {
		   #if 0
		    if(pPPUFunreg->ppuFbGo & (1<<14))
		      regVal = 1;
		   #elif 1
		    if(ppu_lnterlace_flag)
		      regVal = 1; 
		   #endif        
    }
...
EXPORT_SYMBOL(gpHalPPUInterlaceEnget);
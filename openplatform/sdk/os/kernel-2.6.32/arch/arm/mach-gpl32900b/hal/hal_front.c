...
void gpHalFrontIntGet(UINT32 *interrupt)
{
	*interrupt = pFrontReg->frontIntEnable | (pFrontReg->frontVdIntEnable << 8); 
}

void gpHalFrontHdCntSet(UINT32 value)
{
	pFrontReg->frontdummyL = value & 0xFF;
	pFrontReg->frontdummyH = (value >> 8) & 0xFF;
}

void gpHalFrontHdCntGet(UINT32 *value)
{
	*value = (pFrontReg->frontdummyH << 8) | pFrontReg->frontdummyL;
}

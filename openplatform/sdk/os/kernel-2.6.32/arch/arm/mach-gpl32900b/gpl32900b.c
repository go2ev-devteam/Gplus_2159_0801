...
extern void __init gpl32900b_devinit(void); 
extern void gpl32900b_power_off(void);
...
void gpl32900b_mask_irq(unsigned int irq)
{
	if (irq & 0x20) 
	{
...
void gpl32900b_unmask_irq(unsigned int irq)
{
	if (irq & 0x20)
	{
...
int
gpl32900b_irq_wake(unsigned int irqno, unsigned int state)
{	
	if (!state)
	{
	  if (irqno & 0x20)
...
#else
#define gpl32900b_irq_wake NULL
inline void gp_init_pm(void) {}
#endif

static struct irq_chip gpl32900b_irq_chip = {
	.name	= "GPL32900B",
	.mask	= gpl32900b_mask_irq,
	.mask_ack = gpl32900b_mask_irq,
	.unmask = gpl32900b_unmask_irq,
	.set_wake	= gpl32900b_irq_wake,	
};
...
void gp_init_irq(void)
{
	...
	for (i = 0; i < NR_IRQS; i++) {
		set_irq_chip(i, &gpl32900b_irq_chip);
		set_fiq_chip(i, &gpl32900b_irq_chip);
		set_irq_handler(i, handle_level_irq);
		set_irq_flags(i, IRQF_VALID | IRQF_PROBE);
	}
}

static struct map_desc gpl32900b_io_desc[] __initdata = {
	{
		.virtual	= IO0_BASE,
...
{	
	iotable_init( gpl32900b_io_desc, ARRAY_SIZE( gpl32900b_io_desc));
#ifdef CONFIG_PM
	local_flush_tlb_all();
	flush_cache_all();		
    gp_sram_init();
...
extern struct sys_timer gpl32900b_timer;

MACHINE_START(GPL32900B, "GPL32900B")
	.phys_io	= 0x90000000,
	.io_pg_offst	= ((0xfc000000) >> 18) & 0xfffc,
	.boot_params	= 0x00000100,
	.map_io		= gp_map_io,
	.init_irq	= gp_init_irq,
	.init_machine	= gpl32900b_init,
	.timer		= &gpl32900b_timer,
MACHINE_END

...
#define CFG_FPS_AUTO	0x2		//auto change fps, 30fps at day, 25fps at night, cannot work with ldw
...
typedef struct gpIPC_CHN_ATTR_S {
	...
	SINT32 time_length;		//n: reset stream for every n seconds, 0:no reset stream until being requested
	SINT32 max_size;		//max stream size
	SINT32 sps_pps			//1:insert sps pps every I frame 0:only insert sps pss on first I frame (h264 only)
}IPC_Chn_Attr_s;
...
/*****************************************************************
 * description: add osd text to a video channel
 * param: channel [in]: the video stream osd added to, the handle return by gp_IPC_VChn_Open 
 * param: text [in]: ascii string text 
 * param: x [in]: x position related to video frame
 * param: y [in]: y position related to video frame
 * param: font_type [in]: font type index
 * return: A handle to the text.
 * **************************************************************/
UINT32 gp_OSD_AddText(void* channel, UINT8* text, UINT32 x, UINT32 y, UINT32 font_type);

/*****************************************************************
 * description: add osd chinese character to a video channel
 * param: channel [in]: the video stream osd added to, the handle return by gp_IPC_VChn_Open 
 * param: ptr_ch [in]: chinese character
 * param: x [in]: x position related to video frame
 * param: y [in]: y position related to video frame
 * param: str_idx [in]: font type index
 * return: A handle to the text.
 * **************************************************************/
UINT32 gp_OSD_AddTextCh(void* channel, UINT8* ptr_ch, UINT32 x, UINT32 y, UINT32 str_idx, int font_type);

/*****************************************************************
 * description: add osd text to a video channel
 * param: channel [in]: the video stream osd added to, the handle return by gp_IPC_VChn_Open 
 * param: format [in]: time format 0: YYYY/MM/DD 1: MM/DD/YYYY 2: DD/MM/YYYY
 * param: x [in]: x position related to video frame
 * param: y [in]: y position related to video frame
 * param: font_type [in]: font type index
 * param: date_only [in]: 0: date and time 1: date only
 * return: A handle to the text.
 * **************************************************************/
UINT32 gp_OSD_AddTimeStamp(void* channel, UINT32 format, UINT32 x, UINT32 y, UINT32 font_type, UINT32 date_only);

/*****************************************************************
 * description: remove osd text from a video channel
 * param: channel [in]: the video stream osd need to removed from, use the handle return by gp_IPC_VChn_Open 
 * param: handle [in]: the osd text handle need to removed, use the handle return by gp_OSD_AddText
 * return: 0: remove successful -1: cannot remove text (maybe video channel and text don't match)
 * **************************************************************/
SINT32 gp_OSD_RemoveText(void* channel, UINT32 handle);

/*****************************************************************
 * description: remove all osd text in a channel
 * param: channel [in]: the video stream osd need to removed from, use the handle return by gp_IPC_VChn_Open 
 * return: 0: remove successful -1: cannot remove text
 * **************************************************************/
SINT32 gp_OSD_ClearAllText(void* channel);

/*****************************************************************
 * description: enabe/disable all osd text in a channel
 * param: channel [in]: the video stream osd need to enable/disable, use the handle return by gp_IPC_VChn_Open 
 * param: enable [in]: 0: disable 1:enable
 * **************************************************************/
void gp_OSD_Enable(void* channel, SINT32 enable);

/*****************************************************************
 * description: get width and height for an OSD text
 * param: text [in]: the osd text in ascii code.
 * param: font_type [in]: the font type
 * param: width [out]: variable to store width value 
 * param: height [out]: variable to store height value
 * **************************************************************/
void gp_OSD_GetTextSize(UINT8* text, UINT32 font_type, UINT32* width, UINT32* height);

void gp_OSD_GetTextSizeCh(UINT8* text, UINT32 font_type, UINT32* width, UINT32* height, int str_idx);
/*****************************************************************
...
#ifdef ENABLE_QR_SCAN
int gp_QRCode_Scan(UINT8* code, SINT32 len, SINT32 timeout);
#endif
#endif

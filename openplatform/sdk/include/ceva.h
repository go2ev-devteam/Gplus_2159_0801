...
#define VFLAG_GET_INFO_ONLY	0x40000000	// [in]	 decode image info only
#define VFLAG_THUMBNAIL		0x80000000	// [in]	 decode thumbnail
...
#define VFLAG_SMOOTH_DISP	0x00000002 // [in] for On2 H264 smooth display setting
...
#define VFLAG_ARGBOUT       0x00000040  // [in/out] output ARGB instead of YUV
#define VFLAG_RGB565OUT     0x00000100  // [in/out] output RGB565 instead of YUV
...
//ytliao add for 32900B On2
#define VFLAG_YUV422SEMI	0x00000010  // [out] decoder output YUV422 semi-planar format
#define VFLAG_YUV400		0x00000020  // [out] decoder output YUV400 format
#define VFLAG_YUV444SEMI	0x00000080	// [out] decoder output YUV444 semi-planar format
...
#define VFLAG_SEQENDENC     0x00020000  // [in] force encoder to generate sequence end
...
typedef struct {
    ...
    UINT32 forceIFrame;		// [in]     force I Frame
    UINT32 inputFormat;		// [in]		input frame format defined in typedef.h
    UINT32 videoStbOffset;	// [in]		0 (disable video stabilization) / n (enable video stabilization and output (w-n)x(h-n) )

    UINT32 thumbnail;		// [in]		enable/disable jpeg thumbnail
    UINT32 thumbnailWidth;
    UINT32 thumbnailHeight;
    UINT8* thumbnailData;
    UINT32 thumbnailSize;
    
    UINT32 slice; //[in] enable/disable slice encode
    UINT32 slice_height; //[in] slice_height
} VEncode_t;
...
/* function type for dlopen */
typedef SINT32 (*pfCodecLoad)(UINT32 format);
typedef SINT32 (*pfCodecUnload)(void);
typedef SINT32 (*pfCodecInstanceSize)(void);
typedef SINT32 (*pfCodecInit)(void *arg);
typedef SINT32 (*pfCodecExec)(void *arg);
typedef SINT32 (*pfCodecUninit)(void *arg);
typedef cevaCodecType_t (*pfCodecType)(void);
